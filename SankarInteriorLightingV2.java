 ...
 * Mr. Fowler
 */
public class SankarInteriorLightingV2 extends GLCanvas {
	 ...
	 */
	public SankarInteriorLightingV2(GLCapabilities glc){
		super(glc);
	...
	public void doDisplay(GL myGL, int w, int h){
		...
		renderer.draw("Lighting Demonstration (press space for light switch)", 0, 10);
	...
	public void drawLamp(GL myGL){
		GLUT glut = new GLUT();//initialize a GLUT
	...
	public void drawBox(GL myGL){
		...
		for (double i=-roomSize; i<=roomSize; i+=roomSize/steps) {
			for (double j=-roomSize; j<roomSize; j+=roomSize/steps) {
				myGL.glVertex3d(roomSize,j+roomSize/steps,i+roomSize/steps);
				myGL.glVertex3d(roomSize,j,i+roomSize/steps);
				myGL.glVertex3d(roomSize,j,i);
				myGL.glVertex3d(roomSize,j+roomSize/steps,i);
			}
		}
		myGL.glEnd();
		...
		for (double i=-roomSize; i<=roomSize; i+=roomSize/steps) {
			for (double j=-roomSize; j<roomSize; j+=roomSize/steps) {
				myGL.glVertex3d(i+roomSize/steps,j+roomSize/steps,-roomSize);
				myGL.glVertex3d(i+roomSize/steps,j,-roomSize);
				myGL.glVertex3d(i,j,-roomSize);
				myGL.glVertex3d(i,j+roomSize/steps,-roomSize);
			}
		}
		myGL.glEnd();
		...
		myGL.glNormal3d(0, 0, -1);
		myGL.glBegin(GL.GL_QUADS);
		myGL.glMaterialfv(GL.GL_FRONT_AND_BACK, GL.GL_AMBIENT_AND_DIFFUSE,new float[]{0.4f,.2f,.9f,1},0);//change color
		for (double i=-roomSize; i<=roomSize; i+=roomSize/steps) {
			for (double j=-roomSize; j<roomSize; j+=roomSize/steps) {
				myGL.glVertex3d(i+roomSize/steps,j+roomSize/steps,roomSize);
				myGL.glVertex3d(i+roomSize/steps,j,roomSize);
		...
		myGL.glBegin(GL.GL_QUADS);
		myGL.glMaterialfv(GL.GL_FRONT_AND_BACK, GL.GL_AMBIENT_AND_DIFFUSE,new float[]{.8f,.4f,0f,1},0);//change color
		for (double i=-roomSize; i<=roomSize; i+=roomSize/steps) {
			for (double j=-roomSize; j<roomSize; j+=roomSize/steps) {
				myGL.glVertex3d(i+roomSize/steps,roomSize,j+roomSize/steps);
				myGL.glVertex3d(i+roomSize/steps,roomSize,j);
				myGL.glVertex3d(i,roomSize,j);
				myGL.glVertex3d(i,roomSize,j+roomSize/steps);
			}
		}
		myGL.glEnd();
